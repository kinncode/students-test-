...
function showGoogleRegisterForm(user) {
  ...
  // 新增：移除 required 屬性，避免瀏覽器驗證
  document.getElementById('reg-email-modal').removeAttribute('required');
  document.getElementById('reg-password-modal').removeAttribute('required');
  // 預設填入 Google 名字
...
function showExhibitionTab() {
  document.getElementById('main-content').style.display = '';
  // 你可以隱藏個人頁面區塊（如果有）
  document.getElementById('nav-exhibition').classList.add('active');
  document.getElementById('nav-profile').classList.remove('active');
  // 如果有 profile.html 內容，這裡可以用 iframe 或 AJAX 載入
}
function showProfileTab() {
  // 你可以顯示個人頁面區塊（或跳轉 profile.html）
  window.location.href = 'profile.html';
  document.getElementById('nav-exhibition').classList.remove('active');
